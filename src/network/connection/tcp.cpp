...
bool validate_magic(const std::array<uint8_t, 8>& buffer,
...
TCPConnection::TCPConnection(asio::io_context& ctx)
    : socket_(ctx), ctx(ctx), fetcher_(std::make_unique<Web::CMFetcher>()) {}
...
void TCPConnection::network_connect() {
  ...
  STEAMCLIENT_LOG_INFO("resolving server {}:{}", servers[0].first,
                       servers[0].second);
  asio::ip::tcp::resolver resolver(ctx);
  auto endpoints =
      resolver.resolve(servers[0].first, std::to_string(servers[0].second));

  asio::async_connect(socket_, endpoints, [this](std::error_code ec, auto) {
    if (!ec) {
...
void TCPConnection::start_read_header() {
  asio::async_read(socket_, asio::buffer(header_buffer_, 8),
...
void TCPConnection::async_send(const std::vector<uint8_t>& data) {
  ...
  uint32_t data_len =
      boost::endian::native_to_little(static_cast<uint32_t>(data.size()));
  ...
  buffer.insert(buffer.end(), reinterpret_cast<uint8_t*>(&data_len),
