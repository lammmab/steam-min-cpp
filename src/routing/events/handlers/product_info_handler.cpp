...
static Events::ProductInfoResult::ProductAppInfo parse_app(const auto& app) {
  ...
  return {app.appid(), hexlify(app.sha().data(), app.sha().size()),
          vdf["appinfo"].as_object()};
}

static Events::ProductInfoResult::ProductPackageInfo parse_pkg(
    const auto& pkg) {
  const std::string& buffer = pkg.buffer();

  auto vdf = VDF::parse_binary(
      reinterpret_cast<const uint8_t*>(buffer.data()) + 4, buffer.size() - 4);

  return {pkg.packageid(), hexlify(pkg.sha().data(), pkg.sha().size()),
...
static void handle_prod_info_response(
  ...
  for (const auto& app : msg.Body.apps()) apps.push_back(parse_app(app));

  for (const auto& pkg : msg.Body.packages())
    packages.push_back(parse_pkg(pkg));

  Events::ProductInfoResult evt{std::move(apps), std::move(packages)};
