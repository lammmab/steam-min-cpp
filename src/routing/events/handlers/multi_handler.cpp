...
bool read_frame(const std::vector<uint8_t>& payload, size_t& offset,
  ...
  if (offset + subSize > payload.size()) return false;

  out.assign(payload.begin() + offset, payload.begin() + offset + subSize);

  offset += subSize;
  return true;
}

static void handle_multi(CMClient& client,
                         const Packets::PacketClientMsgProtobuf& packet) {
  ClientMessages::MsgProto<CMsgMulti> multi(packet);

  std::vector<uint8_t> payload(multi.Body.message_body().begin(),
                               multi.Body.message_body().end());

  if (multi.Body.size_unzipped() > 0)
    payload = Steam::Utils::GZip::gzip_decompress(payload,
                                                  multi.Body.size_unzipped());

  size_t offset = 0;
  std::vector<uint8_t> frame;

  while (read_frame(payload, offset, frame)) client.consume_frame(frame, false);
}

static Steam::Dispatch::ProtoRegister<Steam::Internal::Enums::EMsg::Multi,
                                      handle_multi>
    reg_multi;