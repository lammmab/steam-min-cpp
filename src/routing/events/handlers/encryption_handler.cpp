...
static void handle_encrypt_request(CMClient& client,
                                   const Packets::PacketMsg& packet) {
  try {
    auto response = client.crypto().generate_encryption_response(packet);
    client.send_msg(response);
  } catch (const std::exception& e) {
    client.emit(
        Events::ChannelSecuredEvent{Events::EventResult::fail(e.what())});
  }
}

static void handle_encrypt_result(CMClient& client,
                                  const Packets::PacketMsg& packet) {
  ...
  Events::ChannelSecuredEvent evt{
      success ? Events::EventResult::ok()
              : Events::EventResult::fail(
                    "Failed to encrypt channel; bad server?")};

  client.emit(evt);
}

static Steam::Dispatch::MsgRegister<Enums::EMsg::ChannelEncryptRequest,
                                    handle_encrypt_request>
    reg_encrypt;

static Steam::Dispatch::MsgRegister<Enums::EMsg::ChannelEncryptResult,
