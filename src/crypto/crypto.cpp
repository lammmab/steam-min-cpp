...
std::vector<uint8_t> EncryptionManager::process_incoming_encrypted_message(
  ...
  return Steam::Crypto::Helpers::symmetric_decrypt_hmac_iv(frame, session_key_,
                                                           hmac_secret_);
}

std::vector<uint8_t> EncryptionManager::process_outgoing_encrypted_message(
    const std::vector<uint8_t>& frame) {
  return Steam::Crypto::Helpers::symmetric_encrypt_hmac_iv(frame, session_key_,
                                                           hmac_secret_, rng_);
}
...
EncryptionManager::generate_encryption_response(
  ...
  ClientMessages::Msg<Steam::Internal::MsgChannelEncryptRequest> request(
      packet);
  std::vector<uint8_t> payload = request.Payload();

  std::vector<uint8_t> challenge(
      payload.begin() + 8,
      payload.end());  // skip the 8 byte protocolVersion and universe

  RUNTIME_ASSERT(request.Body.protocolVersion == 1,
                 "Encryption handshake protocol version mismatch!");
  RUNTIME_ASSERTF(
      request.Body.universe == Steam::Internal::Enums::EUniverse::Public,
      "Expected public universe, but got %d",
      static_cast<int>(request.Body.universe));
  RUNTIME_ASSERTF(challenge.size() == 16,
                  "Expected 16 byte handshake, but got %d bytes",
                  static_cast<int>(challenge.size()));

  std::vector<uint8_t> handshake = session_key_;
  handshake.insert(handshake.end(), challenge.begin(),
                   challenge.end());  // 48 bytes

  std::vector<uint8_t> encrypted_key_vector =
      Steam::Crypto::Helpers::rsa_encrypt_oaep_sha1(getSteamPublicKey(),
                                                    handshake, rng_);
  uint32_t hash_bytes =
      Steam::Crypto::Helpers::crc32_hash(encrypted_key_vector);
