#include <cstdint>
#include <vector>

#include "base/generated/SteamLanguageInternal.hpp"
#include "base/packetbase.hpp"
#include "base/clientmsg.hpp"

#include "crypto/helpers.hpp"
#include "utils/err.h"

namespace Steam::Crypto {
    constexpr uint8_t STEAM_PUBLIC_KEY_DER[] = {
        
    };


    class EncryptionManager {
        public:
            EncryptionManager() 
                : session_key_(Steam::Crypto::Helpers::generate_random_bytes(32,rng_)),
                  hmac_secret_(session_key_.begin(),session_key_.begin()+16)
            {}

            const inline CryptoPP::AutoSeededRandomPool& get_rng() const { return rng_; };
            const inline std::vector<uint8_t>& get_session_key() const { return session_key_; };
            const inline std::vector<uint8_t>& get_hmac_secret() const { return hmac_secret_; };

            inline static std::vector<uint8_t> getSteamPublicKey() {
                return std::vector<uint8_t>(
                    STEAM_PUBLIC_KEY_DER,
                    STEAM_PUBLIC_KEY_DER + sizeof(STEAM_PUBLIC_KEY_DER)
                );
            }

            Steam::Messaging::ClientMessages::Msg<Steam::Internal::MsgChannelEncryptResponse> generate_encryption_response(
                const Steam::Messaging::Packets::PacketMsg& packet
            );

        private:
            CryptoPP::AutoSeededRandomPool rng_; // persistent encryption RNG
            std::vector<uint8_t> session_key_; // 32 byte session key
            std::vector<uint8_t> hmac_secret_; // hmac secret (first 16 bytes of the session_key_)
    };
}