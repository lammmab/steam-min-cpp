...
/*
If GetProductInfo is extended, it can include a vector of app IDs and package IDs.
This will allow for batch results; but I don't need it personally. If you would like to add it;
be my guest.
*/
static void exec_get_product_info(CMClient& client, const Commands::GetProductInfo& req) {
    ClientMessages::MsgProto<CMsgClientPICSProductInfoRequest> msg(Enums::EMsg::ClientPICSProductInfoRequest);
    ...
    // Later, to reduce boilerplate, we will want to do this automatically. Possibly a helper constructor for ProtoMsg ?
