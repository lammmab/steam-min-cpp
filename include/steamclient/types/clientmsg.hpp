...
class Msg
    : public Steam::Messaging::Messages::MsgBaseHdr<Steam::Internal::MsgHdr> {
  static_assert(
      std::is_base_of<Steam::Internal::ISteamSerializableMessage, TBody>::value,
 ...
 public:
  ...
  Msg(const Msg<TBody>& other, int payloadReserve = 0) : Msg(payloadReserve) {
    Header.targetJobID = other.Header.sourceJobID;
  }

  explicit Msg(const Steam::Messaging::Packets::IPacketMsg& packet) {
    const Steam::Messaging::Packets::PacketMsg* packetMsg =
        dynamic_cast<const Steam::Messaging::Packets::PacketMsg*>(&packet);
    if (!packetMsg)
    ...
    if (offset < data.size()) {
      Payload().insert(Payload().end(), data.begin() + offset, data.end());
    }
  }
 ...
 public:
  ...
  Steam::Internal::Enums::EMsg MsgType() const override { return Header.Msg; }
...
class MsgProtoHdr : public Steam::Messaging::Messages::MsgBaseHdr<
                        Steam::Internal::MsgHdrProtoBuf> {
 public:
  explicit MsgProtoHdr(int payloadReserve = 0) {
    Payload().reserve(payloadReserve);
  }

  explicit MsgProtoHdr(Steam::Internal::Enums::EMsg eMsg,
                       int payloadReserve = 64) {
    Payload().reserve(payloadReserve);
    Header.SetEMsg(eMsg);
  }
  ...
  explicit MsgProtoHdr(const Steam::Messaging::Packets::IPacketMsg& packet) {
    const auto* packetMsg =
        dynamic_cast<const Steam::Messaging::Packets::PacketClientMsgProtobuf*>(
            &packet);
...
class MsgProto : public MsgProtoHdr {
 ...
 public:
  ...
  MsgProto(Steam::Internal::Enums::EMsg eMsg,
           const Steam::Messaging::Messages::MsgBaseHdr<
               Steam::Internal::MsgHdrProtoBuf>& msg,
 ...
 public:
  ...
  void SetSourceJobID(uint64_t id) { Header.proto.set_jobid_source(id); }

  void SetTargetJobID(uint64_t id) { Header.proto.set_jobid_target(id); }
};
}  // namespace Steam::Messaging::ClientMessages