...
#include <cstdint>
#include <memory>
#include <steamclient/types/generated/SteamLanguageInternal.hpp>
#include <vector>
...
class PacketClientMsgProtobuf : public IPacketMsg {
 public:
  ...
  PacketClientMsgProtobuf(Steam::Internal::Enums::EMsg eMsg,
...
class PacketClientMsg : public IPacketMsg {
 public:
  ...
  PacketClientMsg(Steam::Internal::Enums::EMsg eMsg,
                  const std::vector<uint8_t>& data)
      : msgType(eMsg), payload(data) {
    Steam::Utils::Stream s(payload);
    header.Deserialize(s);
