...
class IConnection {
 public:
  ...
  virtual void set_on_frame(
      std::function<void(std::vector<uint8_t>)> handler) = 0;

  /// @brief Register a disconect handler.
  ///
  /// The handler is invoked whenever the connection is
  /// dropped for any reason.
  /// @param handler Callback receiving the reason for disconnection
  virtual void set_on_disconnect(
      std::function<void(const std::string& reason)> handler) = 0;
};
}  // namespace Steam::Networking