...
class Stream {
 public:
  ...
  explicit Stream() : readBuffer(nullptr), position(0) {}
  ...
  std::vector<byte> MoveBuffer() { return std::move(buffer); }

  template <typename T>
  void Write(T value) {
    static_assert(std::is_integral_v<T>, "Integral required");

    if (readBuffer) throw std::runtime_error("Stream is in read mode");
  ...
  T Read() {
    ...
    if (!readBuffer) throw std::runtime_error("Stream is in write mode");
  ...
  std::vector<byte> ReadBytes(size_t size) {
    ...
    std::vector<byte> out(readBuffer->begin() + position,
  ...
  std::vector<T> ReadArray(size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) result.push_back(Read<T>());
