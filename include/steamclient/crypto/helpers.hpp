...
inline std::vector<uint8_t> generate_random_bytes(
    size_t length, CryptoPP::AutoSeededRandomPool& rng) {
...
std::vector<uint8_t> rsa_encrypt_oaep_sha1(
    ...
    const std::vector<uint8_t>& plaintext, CryptoPP::AutoSeededRandomPool& rng);
...
void generate_iv(const std::vector<uint8_t>& plaintext,
                 std::vector<uint8_t>& iv,
                 const std::vector<uint8_t>& hmac_secret,
                 CryptoPP::RandomNumberGenerator& rng);

bool validate_iv(const std::vector<uint8_t>& plaintext,
