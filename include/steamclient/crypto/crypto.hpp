...
class EncryptionManager {
 public:
  EncryptionManager()
      : session_key_(Steam::Crypto::Helpers::generate_random_bytes(32, rng_)),
        hmac_secret_(session_key_.begin(), session_key_.begin() + 16) {}

  const inline CryptoPP::AutoSeededRandomPool& get_rng() const { return rng_; };
  const inline std::vector<uint8_t>& get_session_key() const {
    return session_key_;
  };
  const inline std::vector<uint8_t>& get_hmac_secret() const {
    return hmac_secret_;
  };
  ...
  Steam::Messaging::ClientMessages::Msg<
      Steam::Internal::MsgChannelEncryptResponse>
  generate_encryption_response(
